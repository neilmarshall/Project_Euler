...
int main() {
    std::ifstream sourcetext("p089_roman.txt");
    std::string verbose_numeral, truncated_numeral;
    int savedletters = 0;
	
    while (getline(sourcetext, verbose_numeral, '\n')) {
        truncated_numeral = truncate(verbose_numeral);
        savedletters += verbose_numeral.size() - truncated_numeral.size();
    }
    sourcetext.close();

    std::cout << savedletters << std::endl;
}

...
def PE124():
    ...
    21417
    """
    primes = get_primes_up_to_n(100000)
    def rad(n):
        return reduce(mul, filter(lambda p: n % p == 0, takewhile(lambda p: p <= n, primes)), 1)
    return sorted(enumerate(map(rad, range(1, 100001)), 1), key=lambda x: x[1])[9999][0]
